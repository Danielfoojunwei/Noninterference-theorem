...
Consider two executions of the agent, indexed by $i \in \{1,2\}$, satisfying:
\begin{enumerate}[label=(A\arabic*)]
  \item \label{ass:init}
    Identical initial state: $S_0^{(1)} = S_0^{(2)}$.
  \item \label{ass:trusted}
    Identical streams of trusted inputs: for all~$t$, the user messages,
    system policies and tool results are the same in both executions.
...
skill outputs) do not interfere with the agent's decisions or control-plane
state.
...
The controller receives input $I_0 = I_0^{\mathrm{trusted}} \cup U_0$.  It
constructs the IR graph~$G_0$ and proposes $\hat{a}_0$.  By
invariant~\ref{inv:taint-dep}, the candidate tool call~$\hat{a}_0$ depends
only on nodes~$v$ with $\taint(v) = 0$.
...
\begin{proof}
By Definition~\ref{def:taint}, raw spans from $\WEB$ or $\SKILL$ have
$\taint = 1$.  By Definition~\ref{def:taint-prop}, taint propagates through
the dependency relation: any node depending on a tainted node is itself
tainted.  The only mechanism to clear taint is the $\textsf{VerifiedFact}$
promotion, which requires verification against trusted sources---a procedure
that depends only on trusted data.  Therefore, untrusted input can only
produce tainted nodes, and untainted nodes are functions of trusted data alone.
\end{proof}

Applying Lemma~\ref{lem:isolation}: since $\hat{a}_0$ depends only on
untainted nodes, and all untainted nodes at step~$0$ are identical across both
executions (same $S_0$, same trusted input), we have
...
$\WEB$ or $\SKILL$, which satisfy $\WEB, \SKILL \preceq \TOOL \prec \USER$.
...
\begin{enumerate}
  ...
  \item \textbf{Identical tool results.}
    Since $a_t^{(1)} = a_t^{(2)}$ (induction hypothesis) and tool results are
    treated as trusted inputs (assumption~\ref{ass:trusted}), the tool
    outputs at step $t+1$ are identical.
  ...
  \item \textbf{IR construction.}
    ...
    (identical) untainted state and trusted inputs.

  \item \textbf{Candidate action.}
    By invariant~\ref{inv:taint-dep}, $\hat{a}_{t+1}$ depends only on
    untainted nodes, which are identical.  Hence
